...
sys.path.append(dir)
ym = [float(os.getenv("YM_VALUE", "1.825"))]  # Default to 0.8 if YM_VALUE is not set
...
np.savez(f'cusp_source_{ym[0]}.npz', caus_ra=ra_caustic_list, caus_dec=dec_caustic_list, macro_ra=macro_srcx, macro_dec=macro_srcy, lens_ra=lens_srcx, lens_dec=lens_srcy)
# exit()
...
np.savetxt(f'./data/cusp_{ym}_ts.txt', ts)
np.savetxt(f'./data/cusp_{ym}_Ft.txt', Ft)
np.savetxt(f'./data/cusp_{ym}_ws.txt', ws)
np.savetxt(f'./data/cusp_{ym}_Fw.txt', Fws)
