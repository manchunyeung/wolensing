...
kwargs_integrator = {'PixelNum': int(450000),
                     'PixelBlockMax': 2000,
...
ts = np.loadtxt('./data/5m45_ts.txt')
Ft = np.loadtxt('./data/5m45_ft.txt')
amplification.importor(ts=ts, F_tilde=Ft, time=True)
ws, Fws = amplification.fourier(type2=type2)

# np.savetxt('./data/5m45_ts.txt', ts)
# np.savetxt('./data/5m45_ft.txt', Ft)
