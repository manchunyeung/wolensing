...
lens_model_list, kwargs_lens_list = field_injection(zL, zS, MacroImg_ra[microtype], MacroImg_dec[microtype], masses, radius, lens_model_list, kwargs_lens_list, seed=5)
...
import json
with open(f'./data/{filename}_kwargs.txt', 'w') as fout:
        json.dump(kwargs_lens_list, fout)
...
# ts = np.loadtxt('./data/5m13w_ts.txt')
# Ft = np.loadtxt('./data/5m13w_ft.txt')
# amplification.importor(ts=ts, F_tilde=Ft, time=True)
ws, Fws = amplification.fourier(type2=type2)

np.savetxt('./data/7m_ts.txt', ts)
np.savetxt('./data/7m_ft.txt', Ft)
...
plt.legend()
plt.xlim(1, 2000)
plt.savefig(f'./data/{filename}_45abs.png',bbox_inches='tight', dpi=300)
plt.close()
...
plt.legend()
plt.xlim(1, 2000)
plt.savefig(f'./data/{filename}_45pha.png',bbox_inches='tight', dpi=300)
